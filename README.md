...
- 컴포넌트가 한 가지 역할만 하고 있나요? (단일 책임)
  - 각 컴포넌트가 한 가지 역할을 하고 있습니다. (url 경로 렌더링 혹은 부분 컴포넌트 렌더링)
- 굳이 반응형으로 안 만들어도 되지 않았나요? (반응형 남용)
  - 즉각적으로 화면에 반영되어야 하는 데이터들에 반응형을 적용하였습니다.
- API 요청 중·실패 상황을 사용자가 알 수 있게 처리했나요? (로딩·에러 처리)
  - Store 내부의 loading, error 상태 및 로딩 컴포넌트와 요청 실패 시 메시지를 통해 사용자가 알 수 있도록 구현하였습니다.
- 변수·함수 이름만 보고 무엇을 하는지 알기
  - 파일명과 변수명, 함수명 등만 보고 어떤 역할을 하는지 직관적으로 알 수 있도록 구성하였습니다.
